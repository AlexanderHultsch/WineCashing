...
// Owner-Vollansicht inkl. Fortschritt in EINEM Fetch (Amendment, Review-Fix): die
// Owner-UI brauchte started_at/completed_at (z. B. fürs "Abgeschlossen"-Badge) und rief
// dafür zusätzlich GET .../state auf — zwei unabhängige, unsynchronisierte Requests
// konnten bei überlappenden Aufrufen (schnelles Doppel-Öffnen) Route und Progress aus
// zwei verschiedenen Antworten mischen. GET /api/routes/:routeId liefert progress jetzt mit.
export function toRouteWithProgress(route, waypoints, progress) {
  return { ...toRoute(route, waypoints), progress: toProgress(route.id, progress) };
}
